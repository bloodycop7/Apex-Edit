...
local function civilu( text,	xxx, y, texwide )
	surface.SetFont("pd2_assault_text")
	...
	while ( x < texwide ) do
		...
		surface.SetTextColor(Color(233,150,18))
		surface.SetTextPos( x + xxx, y )
		surface.DrawText( text )
...
local moar_w = 230
concommand.Add("jwactive", JW_HUD)
...
function CU()
	...
	if LocalPlayer():IsCP() and GetGlobalBool("cu", false)==true then

		moar_w = math.Clamp(moar_w - 7.5,0,230)


		surface.SetDrawColor(Color(235,158,15,10 + (math.abs(math.sin(CurTime() * 2)) * 45)))

		surface.DrawRect(ScrW() - 310 + moar_w,22,265 - moar_w,46)

		surface.SetDrawColor(Color(235,158,15))
		surface.DrawRect(ScrW() - 310 + moar_w,65,15,3)
		surface.DrawRect(ScrW() - 310 + moar_w,55,3,12)
		...
		surface.DrawRect(ScrW() - 48,24,3,12)
			...
			surface.SetDrawColor(235,158,15)
			surface.SetMaterial(Material("icon16/error.png"))
			surface.DrawTexturedRect(ScrW() - 38,24,16,16)
		...
		if moar_w <= 0 then
			...
			render.SetScissorRect(ScrW() - 310,22,ScrW() - 45,68,true)
			civilu("///     CIVIL UNREST IN EFFECT     ",ScrW() - 300,30,300)
			render.SetScissorRect(0,0,0,0,false)
		end
...
hook.Add("HUDPaint","JW_HUD", JW)
hook.Add("HUDPaint","CU_HUD", CU)