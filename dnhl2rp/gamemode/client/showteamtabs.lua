...
function GM:InformationTab()
	...
	heading:SetText("Welcome to Axenious Community HL2RP Server")
	heading:SetFont("Impulse-Elements19")
...
html:Dock( FILL )
html:OpenURL( "https://docs.google.com/document/d/e/2PACX-1vRM8bdP_28GR-SZRjOWLXquBDkXQm8x45qYgfDYsTJxA6H2A__4T1CbQpmgjwbc2D0t0aG0WcVvAoK_/pub" )
...
function GM:EntitiesTab()
	...
	EntitiesPanel:EnableVerticalScrollbar( true )
		function EntitiesPanel:Update()
			self:Clear(true)
			local UtCat = vgui.Create("DCollapsibleCategory")
			UtCat:SetLabel("Utilities")
				local UtPanel = vgui.Create("DPanelList")
				UtPanel:SetSize(470, 100)
				UtPanel:SetAutoSize(true)
				UtPanel:SetSpacing(1)
				UtPanel:EnableHorizontal(true)
				UtPanel:EnableVerticalScrollbar(true)
			...
			if Utnum ~= 0 then
				UtCat:SetContents(UtPanel)
				UtCat:SetSkin(GAMEMODE.Config.DarkRPSkin)
				self:AddItem(UtCat)
			else
				UtPanel:Remove()
				UtCat:Remove()
...
{name = "Crafting Bench",class = "crafting_table"},
{name = "Glue",class = "glue"},
{name = "Metal Plate",class = "iron"},
{name = "Gunpowder",class = "gunpowder"},
{name = "Piece of Wood",class = "wood"}
}