...
local function ChangeJobVGUI()
    ...
    --		if (!v.Tab) then continue end
         ...
         if v.Tab == F4MenuTabs:GetActiveTab() then
            draw.RoundedBox( 0, 0, 0, w + 4, h + 4, Color( 92, 49, 49) )
        end
    end
end
	end
...
local function KeysMenu(um)
	...
	Frame.Paint = function(self, w, h)
		draw.RoundedBox(0,0,0,w,h,Color(30,30,30,190))
	end
	...
	if trace.Entity:OwnedBy(LocalPlayer()) then
		...
		if (LocalPlayer():IsCP()) then
			...
			local Kickdoor = vgui.Create("DButton", Frame)
			Kickdoor:SetPos(10, Frame:GetTall() - 110)
			Kickdoor:SetSize(180, 100)
			Kickdoor:SetText("Kick door")
			Kickdoor.DoClick = function()
				DermaMenu():AddOption("Kick Door", function() RunConsoleCommand("say", "/kickdoor") Frame:Close() end)
			end
		end
