...
function ENT:Initialize( ) --This function is run when the entity is created so it's a good place to setup our entity.
	...
	self:SetModel( "models/dpfilms/metropolice/elite_police.mdl" ) -- Sets the model of the NPC.
	...
	self:SetSubMaterial(0, "models/impulse/cp/rank_cmd")
...
if ply:IsAdmin() or (RankInfo.xp <= PlayerXP) then
	...
	if DivisionID == 7 then
						...
						local name = "CP:C17-C17:"..DivisionInfo.name.."-"..id
						...
						ply:SetModel(DivisionInfo.model)
						local id = math.random(1002, 9900)
						local name = "CP:C17-C17:"..DivisionInfo.name.."-"..RankInfo.name.."-"..id
					...
					-- Submaterials + Whitelist
					if RankID == 9 and DivisionID == 1 then
						
						if ply:SteamID() != "STEAM_0:1:206522106" and ply:IsAdmin() then
							ply:ChangeTeam( TEAM_CITIZEN, false)
						else
							ply:SetSubMaterial(0, "models/impulse/cp/rank_dco")
					...
					elseif RankID == 8 and DivisionID == 1 then
						if ply:SteamID() != "STEAM_0:1:206522106" and ply:IsAdmin() then
							ply:ChangeTeam( TEAM_CITIZEN, false)
						else
							ply:SetSubMaterial(0, "models/impulse/cp/rank_dvl")
							ply:SetModel("models/dpfilms/metropolice/elite_police.mdl")
						end
					...
					elseif RankID == 7 and DivisionID == 1 then 
						if ply:SteamID() != "STEAM_0:1:206522106" and ply:IsAdmin() then
							ply:ChangeTeam( TEAM_CITIZEN, false)
						else
							ply:SetSubMaterial(0, "models/impulse/cp/rank_sql")
							ply:SetModel("models/dpfilms/metropolice/elite_police.mdl")
						end
					...
					elseif RankID == 6 then 
						if ply:SteamID() != "STEAM_0:1:206522106" and ply:IsAdmin() then
							...
							ply:SetSubMaterial(0, "models/impulse/cp/rank_ofc")
						end
					elseif DivisionID == 7 then
						if ply:SteamID() != "STEAM_0:1:206522106" and ply:IsAdmin() then
							ply:ChangeTeam( TEAM_CITIZEN, false)
						else
							ply:SetSubMaterial(0, "models/impulse/cp/rank_cmd")
						end
					elseif DivisionID == 6 then 
						if !ply:SteamID():match("STEAM_0:0:460056698") or !ply:SteamID():match("STEAM_0:1:206522106") and ply:IsAdmin() then
							ply:ChangeTeam( TEAM_CITIZEN, false)
						else
							ply:SetSubMaterial(0, "models/dpfilms/metropolice/blacop/metrocop_sheet")
						end
					end

					if DivisionID == 5 then
						local id = math.random(1000, 9999)
						local name2 = "CP:C17-C17:"..DivisionInfo.name.."-SqL-"..id
						ply:SetRPName(name2, false)
						ply:SetSubMaterial(0, "models/impulse/cp/rank_sql")
					end

					if DivisionID == 6 then
						if ply:SteamID() != "STEAM_0:0:460056698" then
							local id = math.random(1000, 9999)
							local name2 = "CP:C17-C17:"..DivisionInfo.name.."-OfC-"..id
							ply:SetRPName(name2, false)
						else 
							local id = math.random(1000, 9999)
							local name2 = "CP:C17-C17:"..DivisionInfo.name.."-DvL-"..id
							ply:SetRPName(name2, false)
						end
					end
