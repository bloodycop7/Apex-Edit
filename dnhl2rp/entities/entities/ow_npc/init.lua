...
function ENT:Initialize( ) --This function is run when the entity is created so it's a good place to setup our entity.
	...
	self:SetModel( "models/dpfilms/metropolice/rtb_police.mdl" ) -- Sets the model of the NPC.
...
function giveowWeapons(ply, weapons)
...
if not ply:IsAdmin() then
	...
	if ply:getDarkRPVar("LastOWSet") and 40 - (CurTime() - ply:getDarkRPVar("LastOWSet")) >= 0 then

		ply:notify("You must wait ".. math.ceil(40 - (CurTime() - ply:getDarkRPVar("LastOWSet"))) .. " second('s) before changing Rank or Division.")
...
ply:SetSkin(1)
						...
						ply:UpdateJob("Overwatch Transhuman Arm")
...
ply:SetSkin(1)
				...
				if OWDivisionID == 8 then
					if ply:SteamID() != "STEAM_0:1:206522106" then
						...
						ply:notify("You Are not Whitelisted for this division")
						ply:SetRPName("CHANGE YOUR NAME (/name)")
					else
						ply:notify("You are whitelisted for this division")
					end
				end
