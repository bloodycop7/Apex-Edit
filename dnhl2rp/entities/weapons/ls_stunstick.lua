...
SWEP.IconOverride = "materials/entities/weapon_stunstick.png"
SWEP.Spawnable = true
SWEP.AdminOnly = true
...
function SWEP:DrawHUD()
	...
	self:SetMode(1) 
	if mode == 1 then
		draw.SimpleText("Baton Status: Grounded", "impulseWeapon_CombineFont18", ScrW()/2+40, ScrH()/2+40, Color( 0, 100, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
	elseif mode == 2 then
		draw.SimpleText("Baton Status: Pacify", "impulseWeapon_CombineFont18", ScrW()/2+40, ScrH()/2+40, Color( 0, 100, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
	elseif mode == 3 then
		draw.SimpleText("Baton Status: Active", "impulseWeapon_CombineFont18", ScrW()/2+40, ScrH()/2+40, Color( 0, 100, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
	end
...
function SWEP:SecondaryAttack()
	if self.Owner:KeyDown(IN_WALK) then
		...
		if SERVER then
			...
			if self.Owner:Team() == FACTION_CCA then
