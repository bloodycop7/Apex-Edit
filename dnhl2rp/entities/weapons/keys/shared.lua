...
hook.Add("KeyPress", "combineAutoDoor", function(ply, key)
    if key != IN_USE || (ply.CombineDoorNext && ply.CombineDoorNext > CurTime()) || !ply:IsCP() then return end
    local trace = ply:GetEyeTrace()

    ply.CombineDoorNext = CurTime() + 0.3

    if not IsValid(trace.Entity) or not trace.Entity:IsOwnable() or (trace.Entity.DoorData and trace.Entity.DoorData.NonOwnable) or (trace.Entity:IsDoor() and ply:EyePos():DistToSqr(trace.Entity:GetPos()) > 65*65) or (trace.Entity:IsVehicle() and ply:EyePos():DistToSqr(trace.Entity:NearestPoint(ply:EyePos())) > 100*100) then
        return
    end
    ...
    if trace.Entity:OwnedBy(ply) or (DoorData.GroupOwn and table.HasValue(RPExtraTeamDoors[DoorData.GroupOwn] or {}, Team)) or (DoorData.TeamOwn and DoorData.TeamOwn[Team]) then
        if SERVER then
